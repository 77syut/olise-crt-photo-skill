...
- Static scenes do not receive sports-scale distortion.
- Scanlines, blur, glow, and fisheye read as one CRT surface rather than separate gimmicks.

If classification is wrong, rerun with an explicit motion level. Do not add synthetic motion trails unless the user separately requests them.
